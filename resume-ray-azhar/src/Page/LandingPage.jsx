        ...
        >
            ...
            <Box sx={{ position: 'relative', display: 'inline-block', mt: 1 }}>
                ...
                <Avatar src={person.photo} alt={person.name}
                    sx={{
                        ...
                        position: 'relative', zIndex: 1, fontSize: '2.2rem', bgcolor: person.accentColor,
                    }}>
                    {person.initials}
                </Avatar>
            </Box>
            ...
            <Box>
                <Typography variant="h5" sx={{
                    fontWeight: 800, fontSize: { xs: '1.3rem', sm: '1.5rem' },
                    background: isDark
            ...
            {/* Bio snippet */}
            <Typography sx={{ color: 'text.disabled', fontSize: { xs: '0.82rem', sm: '0.85rem' }, lineHeight: 1.7, maxWidth: 360 }}>
            ...
            <Stack direction="row" flexWrap="wrap" justifyContent="center" gap={1}>
                {person.highlights.map((h) => (
                    <Chip key={h.label}
                        icon={<Box sx={{ color: `${h.color} !important`, fontSize: '0.9rem', display: 'flex' }}><HighlightIcon label={h.label} /></Box>}
                        label={h.label} size="small"
                        sx={{ bgcolor: `${h.color}14`, color: 'text.primary', border: `1px solid ${h.color}30`, fontSize: '0.76rem' }}
                    />
                ))}
            </Stack>
            ...
            <Stack direction="row" spacing={1}>
                {[
                    ...
                    <Box key={item.href} component="a" href={item.href} target={item.external ? '_blank' : undefined}
                        sx={{
            ...
            <Button variant="contained" endIcon={<ArrowIcon />}
                onClick={() => { playClick(); navigate(`/resume/${person.id}`); }}
                fullWidth
                sx={{
                    background: `linear-gradient(135deg, ${person.accentColor}, ${person.secondaryColor})`,
                    borderRadius: 3, textTransform: 'none', fontWeight: 700, fontSize: '0.9rem',
                    py: 1.3, mt: 'auto', boxShadow: `0 8px 24px ${person.accentColor}30`,
                    transition: 'transform 0.2s, box-shadow 0.2s',
                    '&:hover': {
                        background: `linear-gradient(135deg, ${person.secondaryColor}, ${person.accentColor})`,
                        transform: 'translateY(-2px)', boxShadow: `0 12px 32px ${person.accentColor}50`,
                    },
                }}>
                View Resume
            </Button>
    ...
    return (
        <MainLayout>
            ...
            <Box sx={{ position: 'fixed', top: 16, right: 16, zIndex: 200 }} className="no-print">
                <IconButton onClick={toggleMode}
                    sx={{
                        ...
                        '&:hover': { bgcolor: isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.12)' },
                    }}>
            ...
            <Box sx={{
                ...
                px: { xs: 2, sm: 4, md: 6 }, py: { xs: 6, md: 8 },
                ...
                {/* Badge */}
                <Chip label="👋 Welcome to our portfolio" size="small"
                    sx={{
                        ...
                        mb: 3, px: 1, fontSize: '0.85rem', animation: 'fadeInUp 0.5s ease-out',
                    }} />

                {/* Headline */}
                <Typography variant="h1" sx={{
                    fontWeight: 900, fontSize: { xs: '2rem', sm: '2.8rem', md: '3.6rem' },
                    lineHeight: 1.1, mb: 1.5, animation: 'fadeInUp 0.6s ease-out',
                    background: isDark
                ...
                <Typography sx={{
                    fontSize: { xs: '0.95rem', md: '1.1rem' }, color: 'text.disabled',
                    mb: { xs: 5, md: 6 }, animation: 'fadeInUp 0.7s ease-out', maxWidth: 500,
                }}>
                    Meet Raynold and Azhar, two passionate software engineering interns at UNIMAS.
                    Explore their skills, projects, and aspirations in the world of tech.
                </Typography>

                {/* Person Cards */}
                <Box sx={{
                    display: 'flex', flexDirection: { xs: 'column', md: 'row' },
                    gap: { xs: 3, md: 4 }, width: '100%', maxWidth: 1040,
                    justifyContent: 'center', alignItems: { xs: 'center', md: 'stretch' },
